...
const Doctor = ({ doctor }) => {
    return (
        <div className='bg-white p-8 rounded-2xl flex flex-col justify-center gap-4'>
            ...
            <div>
                {
                    doctor.Availability.includes(new Date().toLocaleDateString('en-US', { weekday: 'long' })) ?
                        <button className='btn rounded-full text-green-500 border-green-300 bg-green-50 hover:bg-green-100 mr-4'>
                            Available
                        </button> 
                    :
                        <button className='btn rounded-full text-red-500 border-red-300 bg-red-50 hover:bg-red-100 mr-4'>
                            Not Available
                        </button>
                }
            ...
            <div className='flex flex-col gap-4 pt-4 border-t-2 border-dashed border-[#0F0F0F30]'>
                ...
                <Link to={`/doctors/${doctor.RegistrationNumber}`} className='block'>
                    <button className='btn rounded-full w-full text-lg py-6 text-blue-500 border-blue-300 bg-white hover:bg-blue-100'>
                        View Details
                    </button>
                </Link>
            </div>
